 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Sreevatsa
 *	@version	1.0
 *	@since	10/12/2023
 */ 
public class Wordle
{ 
	...
	{
		String result = "SMART";
		Scanner input1 = FileUtils.openToRead(WORDS5_ALLOWED);
		Scanner input = FileUtils.openToRead(inFileName);
		if (inAllowedWordFile){
			while ( input.hasNext() ) {
				String next = input.next();
				if (testWord.equals(next)) {
					result = testWord;
					if (show) {
			...
			int rand = (int)(Math.random()*1+12972);
			String wordAll = new String[12972];
			for (int i =0; i <wordAll.length;i++) {
				wordAll[i] = input1.next();
			}
			result = wordAll[rand];
	...
	public boolean inAllowedWordFile(String possibleWord)
	{
		...
		while (input.hasNext()) {
				string next = input.next();
				if (possibleWord.equalsIgnoreCase(next)) {
					return true;
				}
		}
		return false;
	...
	public void processGuess ( )
	{
		...
		if (!allowedInWordFile(letters)) {
			JOptionPane pane = new JOptionPane(letters + " is not a word. Press ok to continue ");
			JDialog dial = pane.createDialog(null, "OK");
