 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Sreevatsa Pervela
 *	@version	1.0
 *	@since	10/23/2023
 */ 
public class Wordle
{ 
	...
	public String openFileAndChooseWord(String inFileName, String testWord)
	{
		...
		if(!inAllowedWordFile(testWord)) {
			Scanner openToRead = FileUtils.openToRead(inFileName);
			ArrayList<Object> list = new ArrayList<Object>();
			while(openToRead.hasNext()) {
				list.add(openToRead.next());
			}
			openToRead.close();
			testWord = ((String)list.get((int)(Math.random() * list.size()))).toUpperCase();
		}
		if(show)
	...
	{
		Scanner openToRead = FileUtils.openToRead("words5allowed.txt");
		while(openToRead.hasNext()) {
			if(openToRead.next().equalsIgnoreCase(possibleWord)) {
				openToRead.close();
				return true;
			}
		}
	...
	public void processGuess ( )
	{
		...
		// else if guess is not in words5allowed.txt then print dialog box
			...
			JDialog dialog = 
				new JOptionPane(letters + " is not in word list.").createDialog(null, 
				"INVALID INPUT");
	...
	public void drawPanel() {
        ...
        for (int row = 0; row < 6; row++) {
            ...
            if (wordGuess[row].length() == 5) {
                 boolean[] secondArray = { false, false, false, false, false };
                 boolean[] thirdArray = { false, false, false, false, false };
                for (int col = 0; col < 5; col++) {
                    if (wordGuess[row].charAt(col) == word.charAt(col)) {
                        array[col] = 3;
                        secondArray[col] = (thirdArray[col] = true);
                    }
                }
                for (int k = 0; k < 5; k++) {
                    char character = wordGuess[row].charAt(k);
                    for (int l = 0; l < 5; ++l) {
                        if (!secondArray[k] && !thirdArray[l] && character == word.charAt(l)) {
                            array[k] = 2;
                            secondArray[k] = (thirdArray[l] = true);
                        }
                    }
                }
                for (int n = 0; n < 5; ++n) {
                ...
                for (int index = 0; index < 5; ++index) {
                    char secondChar = wordGuess[row].charAt(index);
                    for (int n2 = 0; n2 < Constants.KEYBOARD.length; n2++) {
                        if (keyBoardColors[n2] < array[index] && 
							Constants.KEYBOARD[n2].charAt(0) == secondChar) {
                            keyBoardColors[n2] = array[index];
                        }
                    }
                }
            }
            for (int n3 = 0; n3 < 5; ++n3) {
                if (wordGuess[row].length() != 0 && array[n3] == 1) {
                    StdDraw.picture(209 + n3 * 68, 650 - row * 68, "letterFrameDarkGray.png");
                }
                else if (wordGuess[row].length() != 0 && array[n3] == 2) {
                    StdDraw.picture(209 + n3 * 68, 650 - row * 68, "letterFrameYellow.png");
                }
                else if (wordGuess[row].length() != 0 && array[n3] == 3) {
                    StdDraw.picture(209 + n3 * 68, 650 - row * 68, "letterFrameGreen.png");
                }
                else {
                    StdDraw.picture(209 + n3 * 68, 650 - row * 68, "letterFrame.png");
                }
            }
        }
        StdDraw.setFont(new Font("Arial", 1, 12));
        StdDraw.picture(350.0, 720.0, "wordle.png");
        StdDraw.setPenColor(StdDraw.LIGHT_GRAY);
        int n4 = 0;
        for (final int[] fourthArray : Constants.KEYPLACEMENT) {
            if (n4 == 19 || n4 == 27 || n4 == 28) {
                StdDraw.picture(fourthArray[0], fourthArray[1], "keyBackgroundBig.png");
            }
            else if (keyBoardColors[n4] == 0) {
                StdDraw.picture(fourthArray[0], fourthArray[1], "keyBackground.png");
            }
            else if (keyBoardColors[n4] == 1) {
                StdDraw.picture(fourthArray[0], fourthArray[1], "keyBackgroundDarkGray.png");
            }
            else if (keyBoardColors[n4] == 2) {
                StdDraw.picture(fourthArray[0], fourthArray[1], "keyBackgroundYellow.png");
            }
            else if (keyBoardColors[n4] == 3) {
                StdDraw.picture(fourthArray[0], fourthArray[1], "keyBackgroundGreen.png");
            }
            StdDraw.setPenColor(StdDraw.BLACK);
            StdDraw.text(fourthArray[0], fourthArray[1], Constants.KEYBOARD[n4]);
            n4++;
        }
	...
	public void checkIfWonOrLost ( )
	{
		...
		// else if all guesses are filled then declare loser
			...
			activeGame = false;
			JDialog secondDialog = 
				new JOptionPane(word + " was the word. Press RESET to begin again").createDialog(null, "Sorry!");
				secondDialog.setLocation(365, 250);
